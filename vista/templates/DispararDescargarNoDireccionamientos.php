...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a  href="index.php?x=018">No Direccionamiento por Fecha</a></li>
  </ol>
</nav>
...
<div class="row">
  <div class="col-md-3 sm-12">
    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
      <a class="btn btn-light" id="v-pills-profile-tab" href="index.php?x=018"><center>Regresar</center></a>
    </div>
    <br>
...
<div class="card ">
  <div class="card-header">
    <H4>No Direccionamiento por Fecha</H4>
  </div>
  <div class="card-body">      
...
//ERROR0006
  if (!empty($_SESSION['NIT_EPS'])){   
     ...
     $sql = "SELECT (SELECT DES_TEM_TOKEN FROM PRS_TEM_TOKEN WHERE TIP_TEM_TOKEN = '".$_POST['var_regimen']."') AS DES_TEM_TOKEN,* FROM PRS_URL_SERVICES WHERE COD_URL = '8' ";
    $params = array();
    ...
    sqlsrv_free_stmt($stmt); 
	    ...
	    echo "<strong>Error!</strong> No se ha logrado autenticar los datos de la compañia, error: ERROR0004</div>";
            ...
            $tamaño_array = json_decode($result, true);
				...
				<a class="list-group-item list-group-item-action list-group-item-info"><span class="badge badge-info badge-pill"><?php echo count($tamaño_array); ?></span> No Direccionamientos encontrados</a>
				</div>
				<div id="global">
				<div id="mensajes">
				<div class="texto">
...
INSERT INTO [dbo].[MIPRES_NO_DIRECCIONAMIENTOS]
     ...
     VALUES
           ...
           ,'".$result[$i]['FecNODireccionamiento']."'
           ,'".$result[$i]['EstNODireccionamiento']."'
           ,'".$result[$i]['FecAnulacion']."'
           ,'N/A'
           ,CURRENT_TIMESTAMP)
END ELSE BEGIN 
UPDATE [dbo].[MIPRES_NO_DIRECCIONAMIENTOS]
   SET 
      ...
      ,[FecNODireccionamiento] = '".$result[$i]['FecNODireccionamiento']."'
      ,[EstNODireccionamiento] = '".$result[$i]['EstNODireccionamiento']."'
      ,[FecAnulacion] = '".$result[$i]['FecAnulacion']."'
...
END";
		            ...
		            echo "<div class='alert alert-danger alert-dismissible'>";
		            echo "<button type='button' class='close' data-dismiss='alert'>&times;</button>";
		            echo "<strong>Error!</strong> Error al guardar el IDNODireccionamiento ".$result[$i]['IDNODireccionamiento']."</div>";
		            }else
		            {
		           echo "<div class='alert alert-success alert-dismissible'>";
		           echo "<button type='button' class='close' data-dismiss='alert'>&times;</button>";
		           echo "<strong>Mensaje:</strong> Se ha completado con exito  ".$result[$i]['IDNODireccionamiento']."</div>";
		            }
		            sqlsrv_free_stmt($stmt);
				    }
				?>
            ...
            <strong>Solicitud con exito!</strong> No se encontraron No Direccionamientos.
            </div>
            <?php
            }
            break;
