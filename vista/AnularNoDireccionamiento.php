		 ...
		 <div class="col-md-9 sm-12">
			<div class="card shadow">
			  ...
			  <div class="card-body">
					...
					<button 
					class="btn btn-outline-dark btn-block" type="submit" onclick="javascript:ListarAnularNoDireccionamiento();">Buscar</button> 
					</div>
					</div>
					<br>
					<div class="form-row">
					<div class="col-md-12 sm-12">
					<div id="ListarAnularNoDireccionamiento"></div>
					</div>
					</div>
			  </div>
			</div>
...
function ListarAnularNoDireccionamiento(){  
document.getElementById('ListarAnularNoDireccionamiento').innerHTML = "<center><div class='spinner-border text-success'></center></div>";
	var var_TI = document.getElementById("var_TI").value;
	var var_NI = document.getElementById("var_NI").value;
	$.post("vista/templates/ListarAnularNoDireccionamiento.php", {var_TI: var_TI,var_NI:var_NI}, function(data){
	$("#ListarAnularNoDireccionamiento").html(data);
	});        
}
...
if (isset($_GET["y"])) {
      switch ($_GET["y"]) {
		...
		ListarAnularNoDireccionamiento();
		</script>
		<?php
        break;
            }
