...
function actualizar($conn){
   ...
   $sql = "SELECT *, CONVERT(VARCHAR(20),FEC_TEM_TOKEN,120) AS FEC_CONSULTA  FROM  PRS_TEM_TOKEN WHERE COD_TEM_TOKEN = '1';";
   $params = array();
...
<div class="card shadow">
  ...
  <div class="card-body">      
      <form class="" action="index.php?x=001&y=001" method="post" >
...
if (isset($_GET["y"])) {
      switch ($_GET["y"]) {
        case '001':
          require_once("services/GET-api-GenerarToken-nit-token-sub.php");
          if (!curl_errno($ch)) {
            switch ($http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE)) {
            case 200:  # OK

            $sql = "UPDATE PRS_TEM_TOKEN SET DES_TEM_TOKEN = ?, USU_TEM_TOKEN = ?, FEC_TEM_TOKEN = CURRENT_TIMESTAMP WHERE COD_TEM_TOKEN = '1';";
