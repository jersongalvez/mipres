		 ...
		 <div class="col-md-9 sm-12">
			<div class="card shadow">
			  <div class="card-header">
			    <H4>Prescripciones por Usuario</H4>
			  </div>
			  <div class="card-body">
					
					<form action="index.php?x=041&y=001" method="post">
					<div class="form-row">
...
if (isset($_GET["y"]))
{
    ...
    {
        case '001':
         $TI = trim($_POST["var_TI"]);
         $NI = trim($_POST["var_NI"]);
    ?>
      <script type="text/javascript">
        $(document).ready(function()
        {
        $("#var_TI").val('<?php echo $TI; ?>');
        $("#var_NI").val('<?php echo $NI; ?>');
        });
      </script>
    <?php
        break;
...
<ul class="nav nav-tabs" role="tablist">
    <li class="nav-item">
      <a class="nav-link active" data-toggle="tab" href="#home">Prescripciones del usuario</a>
    </li>
  </ul>
...
if (!empty($TI) and !empty($NI)) {
	...
	WHERE TIPOIDPACIENTE = '".$TI."' AND NROIDPACIENTE = '".$NI."'
	ORDER BY 2 DESC ";
...
?>							<tr>
                  ...
                  <form class="" action="index.php?x=041&y=001&z=001" method="post">
                  <input type="hidden" class="form-control" name="prs" value="<?php echo trim($row2['NOPRESCRIPCION']);?>"  required>
                  <input type="hidden" class="form-control" name="var_TI" value="<?php echo trim($TI);?>"  required>
                  <input type="hidden" class="form-control" name="var_NI" value="<?php echo trim($NI);?>"  required>
