...
if (!empty($_SESSION['NIT_EPS'])) {
    $sql = "SELECT * FROM PRS_URL_SERVICES WHERE COD_URL = '6'";
    $params = array();
    ...
    if ($row_count === false) {
        ...
        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            $url = $row['DES_URL'] . $_SESSION['NIT_EPS'] . '/' . $_POST['DES_TEM_TOKEN'] . '/' . $_POST['IDDIRECCIONAMIENTO'];

            $ch = curl_init($url);

            $data = array();
            $data["IdDireccionamiento"] = $_POST['IDDIRECCIONAMIENTO'];
            //print_r($data);
